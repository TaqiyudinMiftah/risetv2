...
def _make_encoder(backbone: str = "resnet18", pretrained: bool = False) -> tuple[nn.Module, int]:
    """Create a CNN encoder and return the output feature dimension."""
    if backbone == "resnet18":
        weights = models.ResNet18_Weights.DEFAULT if pretrained else None
        net = models.resnet18(weights=weights)
        dim = 512
        # Replace final FC with identity so we get feature maps
        net.fc = nn.Identity()
        return net, dim
...
class CAERNet(nn.Module):
    ...
    def forward(
        ...
        face_feat = self.face_encoder(face_image)  # [B, face_dim]
        context_feat = self.context_encoder(context_image)  # [B, context_dim]
